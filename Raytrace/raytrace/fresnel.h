...
inline double fresnel_r_s(double cos_theta1, double cos_theta2, double ior1, double ior2)
{
  ...
  float temp = (ior1*cos_theta1 - ior2*cos_theta2) / (ior1*cos_theta1 + ior2*cos_theta2);
  return temp * temp;
}
...
inline double fresnel_r_p(double cos_theta1, double cos_theta2, double ior1, double ior2)
{
  ...
  float temp = (ior1*cos_theta2 - ior2*cos_theta1) / (ior1*cos_theta2 + ior2*cos_theta1);
  return temp * temp;
}
...
inline double fresnel_R(double cos_theta1, double cos_theta2, double ior1, double ior2)
{
  ...
  return (fresnel_r_s( cos_theta1,  cos_theta2,  ior1,  ior2) + fresnel_r_p( cos_theta1,  cos_theta2,  ior1,  ior2) ) *0.5 ;
}
